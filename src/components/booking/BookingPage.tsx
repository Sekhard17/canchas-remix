...
const BookingPage = () => {
  ...
  // Efecto para avanzar automáticamente en móvil
  useEffect(() => {
    if (isMobile) {
      if (bookingData.court && step === 1) {
        setTimeout(() => setStep(2), 500); // Pequeño delay para la animación
      }
  ...
  const handleUpdateBooking = (data: Partial<BookingData>) => {
    setBookingData(prev => {
      ...
      // Si se cambia la fecha, limpiamos la hora seleccionada
      if (data.date) {
        delete newData.time;
      }
      return newData;
